...
type ButtonNativeProps = ButtonSharedProps &
...
function omitSharedProps<T extends ButtonProps>(input: T) {
    const nextProps = { ...input } as T & {
        active?: unknown;
        children?: unknown;
        className?: unknown;
        size?: unknown;
...
const variantClasses: Record<ButtonVariant, string> = {
    primary:
        'border-3 border-brand-primary-light bg-linear-to-r from-[#9446ff] via-[#8440e1] to-[#6b32be] text-[#f0e9ff] shadow-[0_14px_44px_-20px_rgba(132,64,225,0.85)] hover:-translate-y-0.5 hover:from-[#a45dff] hover:via-[#8d47f1] hover:to-[#7f3ee2] hover:shadow-[0_16px_52px_-20px_rgba(132,64,225,1)]',
    secondary:
...
export default function Button(props: ButtonProps) {
    ...
    if ('href' in props && props.href !== undefined) {
        const linkProps = omitSharedProps(props);
    ...
    const buttonProps = omitSharedProps(props) as Omit<
        ButtonNativeProps,
