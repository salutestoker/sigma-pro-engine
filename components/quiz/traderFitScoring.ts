...
function getResultIdFromScore(
    ...
    return matchingBand?.resultId ?? 'free_path';
}
...
function resolveOverrideResult(
    ...
    const highlyAdvanced =
        totalScore >= 34 ||
        (highCapital && highStructure && highTime && highConsistency);

    if (highCapital && highStructure && highTime && highConsistency) {
        return {
            resultId: 'ticker_tactix',
    ...
    if (prefersFlexibility && !highlyAdvanced) {
        return {
            resultId: 'free_path',
    ...
    if (
        ...
        return {
            resultId: 'hybrid_path',
            reason: 'Override: transition profile benefits from a hybrid rollout.',
        };
    }
    ...
    if (moderateExperience && someCapital && systemWillingness) {
        return {
            resultId: 'sigma_ecosystem',
            reason: 'Override: structure-ready profile favors SIGMA ecosystem.',
        };
    }
